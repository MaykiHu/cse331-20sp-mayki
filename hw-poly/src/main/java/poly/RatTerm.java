...
public final class RatTerm {
    ...
    public RatTerm add(RatTerm arg) {
        if (this.expt != arg.expt && !this.isZero() && !arg.isZero() && !this.isNaN() && !arg.isNaN()) {
            throw new IllegalArgumentException("expt must be equivalent or args should be zero or NaN");
        } else {
            return new RatTerm(coeff.add(arg.coeff), Math.max(this.expt, arg.expt)); // Math.max for E: 0
        }
    }
    ...
    public RatTerm antiDifferentiate() {
        int newExpt = expt + 1;
        return new RatTerm(coeff.div(new RatNum(newExpt)), newExpt);
    }
